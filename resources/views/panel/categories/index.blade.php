...
@section('subtitle-page','Módulo de Gestión de Categorias')
...
<div class="card mb-4">
    <div class="card-body">
        <div class="d-flex flex-row-reverse pb-4">
            <a href="{{route('dashboard.categories.create')}}" class="btn btn-primary">Nuevo <i class="fas fa-plus"></i></a>
        </div>
                        ...
                        <tr>
                            ...
                            <td><div class="d-flex justify-content-around">
                                <button class="btn btn-sm px-0 mx-0">
                                    <a href="{{ route('dashboard.categories.show', $category->id) }}" title="Ver"><i class="fas fa-eye fas-icon-purple"></i></a> 
                                </button>
                                <button class="btn btn-sm px-0 mx-0">
                                    <a href="{{route('dashboard.categories.edit',$category->id)}}" title="Editar"><i class="fas fa-edit fas-icon-purple"></i></a>
                                </button>
                                
                                <form action="{{ route('dashboard.categories.update.status', $category) }}" method="post">
                                    @csrf
                                    @method('put')
